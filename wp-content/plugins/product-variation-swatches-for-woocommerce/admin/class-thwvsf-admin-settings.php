...
abstract class THWVSF_Admin_Settings {
	...
	private function output_review_request_link(){
		...
		$is_dismissed = get_transient('thwvsf_review_request_notice_dismissed');
		if($is_dismissed){
			return;
		}
		$is_skipped = get_transient('thwvsf_skip_review_request_notice');
		if($is_skipped){
			return;
		}
		...
		if(!$thwvsf_since){
			...
			$now = time();
			$diff_seconds = $now - $thwvsf_since;
			if($diff_seconds > apply_filters('thwvsf_show_review_request_notice_after', 10 * DAY_IN_SECONDS)){
	...
	private function render_review_request_notice(){
		?>
		<div id="thwvsf_review_request_notice" class="notice notice-info is-dismissible  thpladmin-notice" data-nonce="<?php echo wp_create_nonce( 'thwvsf_review_request_notice'); ?>" data-action="dismiss_thwvsf_review_request_notice" style="display:none">
			<h3>
				<?php _e('Just wanted to say thank you for using Product Variation Swatches plugin in your store.', 'product-variation-swatches-for-woocommerce')?>
			</h3>
			<p><?php _e('We hope you had a great experience. Please leave us with your feedback to serve best to you and others. Cheers!', 'product-variation-swatches-for-woocommerce') ?></p>
			<p class="action-row">
		        <button type="button" class="button button-primary" onclick="window.open('https://wordpress.org/support/plugin/product-variation-swatches-for-woocommerce/reviews/?rate=5#new-post', '_blank')"><?php _e('Review Now', 'product-variation-swatches-for-woocommerce') ?></button>
		        <button type="button" class="button" onclick="thwvsfHideReviewRequestNotice(this)"><?php _e('Remind Me Later', 'product-variation-swatches-for-woocommerce') ?></button>
            	<span class="logo"><a target="_blank" href="https://www.themehigh.com">
                	<img src="<?php echo esc_url(THWVSF_ASSETS_URL_ADMIN.'images/logo.svg') ?>" />
                </a></span>
